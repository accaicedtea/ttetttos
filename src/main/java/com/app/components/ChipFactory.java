...
public final class ChipFactory {
    ...
    public static FlowPane allergenPane(List<String> allergens) {
        ...
        fp.setHgap(5); fp.setVgap(5);
        fp.setAlignment(Pos.CENTER_LEFT);
        fp.getStyleClass().add("prod-allergen-chips");
        if (allergens != null) allergens.forEach(a -> fp.getChildren().add(allergenChip(a)));
        return fp;
    }
    ...
    public static FlowPane ingredientPane(List<String> ingredients) {
        ...
        fp.setHgap(5); fp.setVgap(5);
        fp.setAlignment(Pos.CENTER_LEFT);
        if (ingredients != null) ingredients.forEach(i -> fp.getChildren().add(ingredientChip(i)));
        return fp;
    }

    public enum ChipType { MODAL, CART, PROD }
}
