 ...
 * - Aggiunta persistenza coda tablet su file
 *
 * File locali:
 * /opt/kiosk/current-order.json → ordine corrente (crash recovery)
 * /opt/kiosk/order-queue.json → coda ordini offline
 * /opt/kiosk/tablet-orders-queue.json → coda ordini per tablet KDS (persistente)
 */
public final class OrderQueue {
    ...
    private static final Path TABLET_QUEUE_FILE = resolvePath("tablet-orders-queue.json");
    ...
    // --- TABLET QUEUE (MUTUALLY EXCLUSIVE) ---
    private static final List<JsonObject> tabletOrders = Collections.synchronizedList(new ArrayList<>());

    static {
        // Carica gli ordini persistenti all'avvio
        loadTabletOrders();
    }

    /**
     * Carica la coda di ordini da file persistente
     */
    private static void loadTabletOrders() {
        if (!Files.exists(TABLET_QUEUE_FILE)) {
            System.out.println("[OrderQueue] Tablet orders file not found, starting with empty queue");
            return;
        }
        try {
            String content = Files.readString(TABLET_QUEUE_FILE);
            JsonArray arr = JsonParser.parseString(content).getAsJsonArray();
            synchronized(tabletOrders) {
                tabletOrders.clear();
                for (int i = 0; i < arr.size(); i++) {
                    tabletOrders.add(arr.get(i).getAsJsonObject());
                }
            }
            System.out.println("[OrderQueue] Loaded " + tabletOrders.size() + " pending orders from file");
        } catch (Exception e) {
            System.err.println("[OrderQueue] Error loading tablet orders from file: " + e.getMessage());
        }
    }

    /**
     * Salva la coda di ordini su file persistente
     */
    private static void saveTabletOrders() {
        try {
            synchronized(tabletOrders) {
                ...
                Files.writeString(TABLET_QUEUE_FILE, new GsonBuilder().setPrettyPrinting().create().toJson(arr));
            }
        } catch (Exception e) {
            System.err.println("[OrderQueue] Error saving tablet orders to file: " + e.getMessage());
        }
    }

    /**
     * Enqueue a new order for the Android tablets and notify all connected WebSockets.
     */
    public static void enqueueForTablets(JsonObject payload, String generatedId) {
        JsonObject tabletOrder = payload.deepCopy();
        tabletOrder.addProperty("tablet_order_id", generatedId);
        
        tabletOrders.add(tabletOrder);
        saveTabletOrders(); // ✅ Salva su file
        System.out.println("[OrderQueue] Added to tablet queue: " + generatedId);
        
        // Broadcast the updated queue to all connected websockets
        com.api.services.LocalServerManager.broadcastTabletOrders();
    }

    /**
     * Called when a tablet wants to "take" an order. Removes it from the list.
     * @return true if successful (first to claim), false if already taken.
     */
    public static boolean takeOrderFromTablets(String tabletOrderId) {
        synchronized(tabletOrders) {
            for (int i = 0; i < tabletOrders.size(); i++) {
                JsonObject order = tabletOrders.get(i);
                if (order.has("tablet_order_id") && order.get("tablet_order_id").getAsString().equals(tabletOrderId)) {
                    tabletOrders.remove(i);
                    saveTabletOrders(); // ✅ Salva su file dopo rimozione
                    return true;
                }
            }
            return false;
    ...
    public static void createOrderAsync(CartManager cart, String method, Runnable onSuccess, java.util.function.Consumer<String> onError) {
        String pagamento = "contanti";
        if ("card".equals(method) || "bancomat".equals(method)) {
            pagamento = "pos_esterno";
        ...
        EXECUTOR.submit(() -> {
            try {
                // Genera un ID dummy per i tablet in tempo reale usando lo pseudocodice di time
                String tabletId = "RT-" + System.currentTimeMillis();
                enqueueForTablets(payload, tabletId);
    ...
    private static String extractNumeroOrdine(JsonObject resp) {
        try {
            // Cerca nel root della risposta prima di cercare in 'data'
            if (resp.has("ordine_id")) {
                int id = resp.get("ordine_id").getAsInt();
                System.out.println("[OrderQueue] Extracted ordine_id from root: " + id);
            ...
            if (resp.has("data")) {
                ...
                if (data.has("ordine_id")) {
                    int id = data.get("ordine_id").getAsInt();
                    System.out.println("[OrderQueue] Extracted ordine_id from data: " + id);
                    return String.valueOf(id);
                }
                if (data.has("numero_ordine")) {
                    String num = data.get("numero_ordine").getAsString();
                    System.out.println("[OrderQueue] Extracted numero_ordine: " + num);
                    return num;
                }
            }
        } catch (Exception e) {
            System.err.println("[OrderQueue] Error extracting ordine_id: " + e.getMessage());
        }
        
        // Fallback: genera ID dummy solo se completamente fallisce
        String fallback = String.valueOf(localSeq.incrementAndGet());
        System.out.println("[OrderQueue] Using fallback ID: " + fallback);
        return fallback;
    }
}
